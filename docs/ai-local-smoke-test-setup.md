...
Backend에서 video stage1 detect를 subprocess로 분리 실행할 때는 `VERIFAKE_VIDEO_AI_PYTHON`을 지정할 수 있습니다. 미설정 시 backend 서버를 실행한 Python을 사용합니다.

```bash
export VERIFAKE_VIDEO_AI_PYTHON="$PWD/.venv-video/bin/python"
```
